#!/usr/bin/env python3
"""Deep purged WF v1 — S2 shortlist vs MOM_ONLY (correctness re-receipt).

KaggleMLEngine / Synap copper FinPredict.
promote=false ALWAYS. Fail closed. HG proxy labels.

v0 (2026-09-20) is KILL. This is not “more trees on the same hyp.”
v1 fixes two P&L bugs, then re-stamps the same gate:

- Cost: charge on **turnover**, not every week in a position.
  One-way = (RT 4 bps + 2 bps/side slip) / 2 = 4 bps.
- Labels: drop ``fwd_ret == 0`` (v0 coded flats as class 0 / down).

Robustness mode (``DEEP_TEST_MODE=robustness``) repeats the primary
suite across extra seeds. That is the only justified extra compute on
this family. Do not raise ``n_estimators`` to “run for hours.”

Kaggle: paste this file as one cell, or run ``main()``.
Local: ``python scripts/synap_copper/deep_test_s2_wf_v1.py``.
"""
...
OUT_DIR = Path(
    os.environ.get("DEEP_TEST_OUT", str(ROOT / "docs" / "kaggle_deep_test_s2_wf_v1"))
)
...
MODE = os.environ.get("DEEP_TEST_MODE", "v1").strip().lower()
if MODE not in {"v1", "robustness"}:
    raise ValueError(f"DEEP_TEST_MODE must be v1 or robustness, got {MODE!r}")
PRIMARY_SEED = 42
ROBUSTNESS_SEEDS = (42, 43, 44, 45, 46) if MODE == "robustness" else (42,)
...
def main() -> int:
    ...
    metrics = {
        ...
        "v0_verdict": "KILL",
        "v1_changes": [
            "cost_model=turnover (v0 charged COST every week in position)",
            "drop fwd_ret==0 (v0 labeled flats as down)",
            "xgb required import; gpu fallback only on ValueError/RuntimeError/TypeError",
            "gpu claimed only if nvidia-smi AND torch.cuda",
        ],
        "verdict": verdict,
        "built_at_utc": built,
        "panel": {
    ...
    (OUT_DIR / "deep_test_s2_metrics.json").write_text(
        json.dumps(metrics, indent=2, default=str)
    )
    (OUT_DIR / "deep_test_s2_fold_table.json").write_text(
    ...
    pd.DataFrame(rows).to_csv(OUT_DIR / "deep_test_s2_fold_table.csv", index=False)
    ...
    receipt_lines = [
        f"Verdict: {verdict} (promote=false) version={VERSION} mode={MODE}",
        ...
        f"Path: {OUT_DIR / 'deep_test_s2_metrics.json'}",
    ]
    receipt = "\n".join(receipt_lines) + "\n"
    ...
    memo = {
        ...
        "headline": headline,
        "paths": {
            "metrics": str(OUT_DIR / "deep_test_s2_metrics.json"),
            "folds": str(OUT_DIR / "deep_test_s2_fold_table.csv"),
            "receipt": str(OUT_DIR / "ML4T_RECEIPT_CARD.md"),
        },
    }
    (OUT_DIR / "deep_test_s2_memo.json").write_text(json.dumps(memo, indent=2, default=str))
